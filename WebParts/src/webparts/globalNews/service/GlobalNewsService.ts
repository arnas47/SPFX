...
export class GlobalNewsService{
    ...
    public loadMoreLatestNewsData(prevAllData: AllData, siteUrl: string): Promise<Data>{
        ...
        .then(response => {
            ...
            var allItems: Item[] = prevAllData.latestNews.items.concat(latestNews);

            let data: Data = {
                items: allItems,
                excluded: this.itemsToExclude(lastDate, allItems, [
                    GlobalNewsConst.newsContentTypeId, 
                    GlobalNewsConst.appointmentContentTypeId, 
                    GlobalNewsConst.blogContentTypeId,
                    GlobalNewsConst.spnContentTypeId,
    ...
    public loadMoreOtherNewsData(prevData: Data, siteUrl: string, contentTypeId: string): Promise<Data>{
        ...
        .then(response => {
            ...
            items = items.splice(0, this.rowLimit);
            var lastDate = items[items.length - 1].kemiraDateTimeRefinable;
            var allItems: Item[] = prevData.items.concat(items);
            
            let data: Data = {
                items: allItems,
                excluded: this.itemsToExclude(lastDate, allItems, [contentTypeId]),
                outOfData: ofd
            };
    ...
    private itemsToExclude(lastDate: string, items: Item[], contentTypeIds: string[]): Item[]{
        ...
        var lastDataPart = lastDate.split("T")[0];

        for(var i = 0; i < items.length; i++){
            if(items[i].kemiraDateTimeRefinable.split("T")[0] == lastDataPart && contentTypeIds.indexOf(items[i].ContentTypeId) >= 0){
    ...
    public static checkLocalStorageForChanges(type: string, responseData: Data): boolean{
        if (typeof(Storage) !== "undefined") {
            var storageData: Data = GlobalNewsCache.getStorageItem(type);

            if(storageData != null){
                if(responseData.items.length === storageData.items.length){
                    var coincide = true;

                    for(var i = 0; i < storageData.items.length; i++){
                        if(storageData.items[i].Id !== responseData.items[i].Id || responseData.items[i].ContentTypeId !== responseData.items[i].ContentTypeId){
                            coincide = false;
                            break;
                        }
                    }
    ...
    public static itemsPerRowCalculation(): number{
        var contentWidth: number = document.getElementById("workbenchPageContent").clientWidth;
        console.log(contentWidth);

        if(contentWidth > 923 && contentWidth <= 1200){
            return 5;
        }
        else if(contentWidth > 767 && contentWidth <= 923){
            return 4;
        }
        else if(contentWidth > 480 && contentWidth <= 767){
            return 3;
        }
        else if(contentWidth > 320 && contentWidth <= 480){
            return 2;
        }
